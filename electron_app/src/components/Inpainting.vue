<template>
  <div class="animatable_content_box">
    <div class="content_toolbox" style="margin-bottom: -13px">
      ...
      <div
        v-if="stable_diffusion.is_input_avail && inp_img"
        style="float: right; margin-right: 10px"
      >
        <input
          v-model="stroke_size_no"
          style="zoom: 0.8; margin-top: 7px; width: 100px"
          type="range"
          min="1"
          max="100"
        />
    ...
    <div class="inapint_container" @drop.prevent="onDragFile" @dragover.prevent>
      <ImageCanvas
        ...
        id="inpaint"
        canvas_id="inpaintcan"
        canvas_d_id="inpaintcand"
        ...
        <center>
          <p
            style="
              ...
              padding-bottom: calc(50vh - 155px);
              opacity: 70%;
            "
          >
    ...
    <div
      v-if="
        ...
        stable_diffusion.generated_by == 'inpainting'
      "
    >
      <LoaderModal
  ...
  methods: {
    generate(is_retry) {
      ...
      if (is_retry && this.retry_params) {
        ...
        input_image = window.ipcRenderer.sendSync(
          "save_b64_image",
      ...
      let callbacks = {
        on_img(img_path) {
          ...
          if (that.history_key) {
            let p = {
              prompt: "Inpainting: " + that.prompt,
              key: that.history_key,
              imgs: [img_path],
              inp_img: that.input_img_orig,
