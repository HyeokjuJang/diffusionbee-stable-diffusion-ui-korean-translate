  ...
  <div class="animatable_content_box">
    <div class="left_half">
      ...
      <div
        ...
        @dragover.prevent
        class="image_area"
        :class="{ crosshair_cur: is_inpaint }"
        style="height: calc(100% - 200px); border-radius: 16px; padding: 5px"
      >
        <ImageCanvas
          ref="inp_img_canvas"
          :is_inpaint="is_inpaint"
          :image_source="inp_img"
          :is_disabled="!stable_diffusion.is_input_avail"
          canvas_id="img2imgcan"
          canvas_d_id="img2imgcand"
        ></ImageCanvas>
      </div>
      <div
        v-else
        ...
        class="image_area"
        :class="{ pointer_cursor: is_sd_active }"
        style="height: calc(100% - 200px); border-radius: 16px; padding: 5px"
      >
        <center>
      ...
      >
        ...
        <SDOptionsDropdown
          ...
          :elements_hidden="['img_h', 'img_w']"
        >
        </SDOptionsDropdown>
      </div>
      <div
        v-else-if="stable_diffusion.generated_by == 'img2img'"
        class="content_toolbox"
        style="margin-top: 10px; margin-bottom: -10px"
      >
        <div
    ...
    <div class="right_half">
      ...
      <div
        v-if="
          ...
          stable_diffusion.generated_by == 'img2img'
        "
      >
        <LoaderModal
...
export default {
  ...
  methods: {
    generate_img2img() {
      ...
      let input_image = window.ipcRenderer.sendSync(
        "save_b64_image",
        this.$refs.inp_img_canvas.get_img_b64()
      );
      let input_image_with_mask;
      let mask_img;
      if (this.is_inpaint) {
        input_image_with_mask = window.ipcRenderer.sendSync(
          "save_b64_image",
          this.$refs.inp_img_canvas.get_img_mask_bg4()
        );
        mask_img = window.ipcRenderer.sendSync(
          "save_b64_image",
          this.$refs.inp_img_canvas.get_mask_b46(),
          true
        );
      } else {
      ...
      if (this.is_inpaint) params["mask_image"] = mask_img;
      ...
      let callbacks = {
        on_img(img_path) {
          ...
          let p = {
            ...
            key: history_key,
            imgs: [],
            inp_img: input_image_with_mask,
            dif_steps: that.dif_steps,
            inp_img_strength: that.inp_img_strength,
            model_version: that.stable_diffusion.model_version,
    ...
    open_input_image() {
      if (!this.stable_diffusion.is_input_avail) return;
