...
<template>
  <div class="animatable_content_box">
    ...
    <div
      ...
      class="l_button"
    >
      {{ this.app_state.show_history_in_oldest_first ? "오래된" : "새로운" }}
      것 우선
    </div>
    <div
      @click="clear_history()"
    ...
    <div v-if="Object.values(app_state.app_data.history).length > 0">
      ...
      <div
        v-for="history_box in get_history.slice(
          (currentPage - 1) * 30,
          currentPage * 30
        )"
        :key="history_box.key"
      ...
      >
        <div
          @click="delete_hist(history_box.key)"
        ...
        >
          <template #button-content>
            <div class="l_button">공유</div>
          </template>
          <b-dropdown-item-button @click="share_on_arthub(history_box)"
            >ArtHub.ai에 공유하기</b-dropdown-item-button
          >
        </b-dropdown>

        <p class="history_box_info text_bg" style="user-select: text">
          <img
            v-if="history_box.inp_img"
            :src="'file://' + history_box.inp_img"
            style="height: 50px"
          />
          <br v-if="history_box.inp_img" />
          <br v-if="history_box.inp_img" />

          <span style="opacity: 0.5" v-if="get_box_params_str(history_box)">
            {{ get_box_params_str(history_box) }}
          </span>
        ...
        <div v-for="img in history_box.imgs" :key="img" class="history_box">
          <ImageItem
            :app_state="app_state"
            :hide_extra_save_button="true"
            :path="img"
            :style_obj="{ height: '100%' }"
          ></ImageItem>
        </div>
        <div style="clear: both; display: table; margin-bottom: 10px"></div>
...
export default {
  ...
  computed: {
    get_history() {
      ...
      const that = this;
      const list = this.app_state.show_history_in_oldest_first
        ? history
        : history.reverse();
  ...
  methods: {
    ...
    toggle_order() {
      Vue.set(
        this.app_state,
        "show_history_in_oldest_first",
        !this.app_state.show_history_in_oldest_first
      );
    },
    ...
    get_box_params_dict(box) {
      ...
      let vals = {
        seed: "Seed",
        guidence_scale: "Scale",
        dif_steps: "Steps",
        inp_img_strength: "Image Strength",
        img_w: "Img Width",
        img_h: "Img Height",
        negative_prompt: "Negative Prompt",
        model_version: "model_version",
      };
      for (let k in vals) if (box[k]) r[vals[k]] = box[k];
      return r;
    },
    ...
    share_on_arthub(box) {
      ...
      let that = this;
      share_on_arthub(box.imgs, params, box.prompt)
        .then(function () {
