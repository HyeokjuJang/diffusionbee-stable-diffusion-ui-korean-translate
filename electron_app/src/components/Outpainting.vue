<template>
  <div class="animatable_content_box">
    <div class="content_toolbox" style="margin-bottom: 5px; margin-top: -5px">
      ...
      <div
        class="l_button"
        v-if="is_stage_modified && stable_diffusion.is_input_avail"
        style="float: right"
        @click="save_image"
      >
        저장
      ...
      <div
        class="l_button"
        v-if="stable_diffusion.is_input_avail && is_stage_modified"
        style="float: right"
        @click="
          is_eraser_enabled = !is_eraser_enabled;
    ...
    <div
      v-if="
        ...
        stable_diffusion.generated_by == 'outpainting'
      "
    >
      <LoaderModal
...
export default {
  ...
  methods: {
    init_state() {
      let that = this;
      this.stage_h =
        this.stage_w / (window.innerWidth / (window.innerHeight - 230));
      ...
      box.on("mouseover", function () {
        if (!that.is_eraser_enabled) document.body.style.cursor = "move";
      ...
      onVisible(
        document.querySelector("#outpainting_container"),
        this.resize_stage
      );
    ...
    init_eraser() {
      ...
      this.stage.on("mousedown touchstart", function () {
        if (!that.is_eraser_enabled) return;
        ...
        that.undo_history.push({
          alpha: that.lastLine_alpha,
          img: that.lastLine,
        });
    ...
    get_img_of_box(is_mask = false) {
      ...
      let layer;
      if (is_mask) layer = this.alpha_layer;
      else layer = this.images_layer;
      ...
      let img_url = window.ipcRenderer.sendSync(
        "save_b64_image",
        data_uri,
        true
      );
    ...
    save_image() {
      ...
      if (!out_path) return;
      let org_path = window.ipcRenderer.sendSync(
        "save_b64_image",
        img_b64,
        true
      );
