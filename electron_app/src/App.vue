...
<template>
  <div id="app">
    ...
    <ApplicationFrame
      ...
      @menu_item_click_model_license="
        open_url('https://diffusionbee.com/MODEL_LICENSE.txt')
      "
      @menu_item_click_oss_license="
        open_url('https://diffusionbee.com/OPEN_SOURCE_LICENSES.txt')
      "
    >
      <template v-slot:txt2img>
        <ImgGenerate
          v-if="is_mounted && stable_diffusion.is_backend_loaded"
          :app_state="app_state"
          :stable_diffusion="stable_diffusion"
        ></ImgGenerate>
        <div v-else class="animatable_content_box">
          <LoaderModal
            :loading_percentage="stable_diffusion.loading_percentage"
            :loading_desc="stable_diffusion.model_loading_msg"
            :loading_title="
              stable_diffusion.model_loading_title || 'Loading Model'
            "
          >
          </LoaderModal>
    ...
    <LoaderModal
      v-if="app_state.global_loader_modal_msg"
      :loading_percentage="-1"
      :loading_title="app_state.global_loader_modal_msg"
    >
    </LoaderModal>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    if (
      require("../package.json").is_dev ||
      require("../package.json").build_number.includes("dev")
    )
      alert("Not checking for updates.");
    else this.check_for_updates();
  ...
  methods: {
    ...
    check_for_updates() {
      ...
      let updates_url =
        "https://aeyfmzu2ac.execute-api.us-east-1.amazonaws.com/check_diffusionbee_updates?user_id=" +
        user_id;
      xmlHttp.onreadystatechange = function () {
        if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
          let latest_app_version = xmlHttp.responseText.split("|")[0];
          console.log(
            "Latest app version" + latest_app_version + " " + user_id
          );
          let current_versoin =
          ...
          let current_build_no = Number(
            require("../package.json").build_number
          );
          if (
            latest_app_version != current_versoin &&
            latest_build_no > current_build_no
          ) {
            if (
              native_confirm(
                "A new version of " +
    ...
    set_show_dialog_on_quit() {
      ...
      if (!this.$refs.stable_diffusion.is_backend_loaded) {
        ...
        this.show_dialog_on_quit_msg =
          "Model has not finished loading. Are you sure you want to quit?";
        window.ipcRenderer.sendSync(
          "show_dialog_on_quit",
          this.show_dialog_on_quit_msg
        );
        ...
        this.should_show_dialog_on_quit = true;
        this.show_dialog_on_quit_msg =
          "Images are still being generated. Are you sure you want to quit?";
        window.ipcRenderer.sendSync(
          "show_dialog_on_quit",
          this.show_dialog_on_quit_msg
        );
